...
export const TimesheetFooter = ({ readOnly = false, tracksAttendance, contractParts, totals }: TimesheetFooterProps) => {
  ...
  return (
    <div className={cn("grid grid-cols-subgrid col-[1/-1] sticky bottom-0 z-20 self-end bg-slate-100 font-bold border-t-2 border-slate-300", readOnly && "pointer-events-none select-none opacity-80")}>
      ...
      {tracksAttendance && (
        <>
          <div className={cn(cell, !totals.workedHoursMeetsObligation ? "text-red-600" : "text-blue-800")}>
      ...
      <div className={cn(centered, !totals.coreHoursWithinTolerance ? "text-red-600" : "text-blue-800")}>
      ...
      {contractParts.map((part) => {
        ...
        const matches = total?.matchesObligation ?? false;
        return (
          <div key={part.id} className={cn(centered, !matches ? "text-red-600" : "text-blue-800")}>
