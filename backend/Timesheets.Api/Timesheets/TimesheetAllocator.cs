...
internal static class TimesheetAllocator
{
    public static TimesheetAllocation Allocate(TimesheetDraftContext context, TimesheetDraft draft, int? dayNumber)
    {
        TimesheetDraftSnapshot snapshot = TimesheetDrafts.BuildSnapshot(context, draft);
        IReadOnlyList<TimesheetDraftDayState> days = dayNumber.HasValue
            ? snapshot.Days.Where(day => day.Date.Day == dayNumber.Value).ToArray()
            : OrderDays(snapshot.Days);

        Dictionary<Guid, decimal> targets = snapshot.Projects.ToDictionary(
            project => project.Id,
            project => Math.Max(0m, MonthlyTarget(snapshot, project.Workload) - snapshot.Days.Sum(value => value.ProjectHours.GetValueOrDefault(project.Id))));
        ...
        List<TimesheetAllocationDay> allocation = snapshot.Days
            .Select(day => new TimesheetAllocationDay(day.Date, day.CoreHours, day.ProjectHours))
            .ToList();

        return new TimesheetAllocation(allocation, TimesheetDrafts.Evaluate(context, snapshot));
    }

    private static void AllocateDay(
        TimesheetDraftDayState day,
        IReadOnlyList<TimesheetDraftProjectState> projects,
        decimal totalWorkload,
        ref decimal coreTarget,
        Dictionary<Guid, decimal> projectTargets)
    {
        ...
        decimal allocated = day.CoreHours + day.ProjectHours.Values.Sum();
        decimal free = TimesheetLogic.Normalize(capacity - allocated);
        if (free <= 0)
        {
            return;
        }
        ...
        for (int index = 0; index < remaining.Count; index++)
        {
            ...
            decimal value = index == remaining.Count - 1
                ? left
                : TimesheetLogic.Normalize(amount * target / totalRemaining);
    ...
    private static decimal DayCapacity(TimesheetDraftDayState day, decimal totalWorkload)
    {
        ...
        return TimesheetLogic.IsWorkday(day.Date, day.IsHoliday) || !string.IsNullOrWhiteSpace(day.Description)
            ? TimesheetLogic.Normalize(8m * totalWorkload)
            : 0m;
    }
    ...
    private static IReadOnlyList<TimesheetDraftDayState> OrderDays(IReadOnlyList<TimesheetDraftDayState> days) =>
        days.OrderByDescending(day => !string.IsNullOrWhiteSpace(day.Description))
